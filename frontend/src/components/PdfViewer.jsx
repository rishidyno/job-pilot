 ...
 * Responsive, accessible, with Escape key support.
 */
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
      ...
      <div className={`bg-white flex flex-col ${
        fullscreen
          ...
          : 'w-full h-[85vh] sm:h-[90vh] sm:max-w-5xl rounded-t-2xl sm:rounded-xl'
      }`}>
        {/* Toolbar */}
        <div className="flex items-center justify-between px-3 sm:px-4 py-3 border-b border-gray-200 shrink-0">
          <h3 className="text-sm font-semibold text-gray-700 truncate max-w-[40%] sm:max-w-none">{title || 'Resume Preview'}</h3>
          <div className="flex items-center gap-0.5 sm:gap-1">
            <button onClick={() => setZoom(z => Math.max(50, z - 25))}
              className="p-1.5 hover:bg-gray-100 rounded-lg" aria-label="Zoom out">
              <ZoomOut className="w-4 h-4 text-gray-500" />
            </button>
            <span className="text-xs text-gray-400 w-10 text-center">{zoom}%</span>
            <button onClick={() => setZoom(z => Math.min(200, z + 25))}
              className="p-1.5 hover:bg-gray-100 rounded-lg" aria-label="Zoom in">
              <ZoomIn className="w-4 h-4 text-gray-500" />
            </button>
            <div className="w-px h-5 bg-gray-200 mx-0.5 sm:mx-1 hidden sm:block" />
            <button onClick={() => setFullscreen(f => !f)}
              className="p-1.5 hover:bg-gray-100 rounded-lg hidden sm:block"
            ...
            <button onClick={onClose} className="p-1.5 hover:bg-gray-100 rounded-lg ml-0.5 sm:ml-1"
